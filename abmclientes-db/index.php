...
if ($_POST) {
    ...
    if ($id != "") {
        ...
        //Si no se subió la imagen, mantengo el nombre actual que ya existia de la imagen
        if ($_FILES["archivo"]["error"] != UPLOAD_ERR_OK) {
            $nuevoNombre = $aClientes["imagen"];
        } else {
            //Si viene la imagen, elimino la imagen anterior y guardo el nombre de la nueva imagen
            unlink("imagenes/" . $aClientes[$id]["imagen"]);
            //Nota: para eliminar un archivo podemos usar diapositiva 33 unlink()
        }
        //Actualiza un cliente
        $aClientes[$id] = array("dni" => $dni, "nombre" => $nombre, "telefono" => $telefono, "correo" => $correo, "imagen" => $nuevoNombre);
    } else {
        //La primera pregunta si se cargo el archivo?
        //linea de codigo optimiada

        //Inserta un nuevo cliente
        $aClientes[] = array("dni" => $dni, "nombre" => $nombre, "telefono" => $telefono, "correo" => $correo, "imagen" => $nuevoNombre);
    }

    //Convertir dicho array a json
...
if ($id != "" && isset($_REQUEST["do"]) && $_REQUEST["do"] == "eliminar") {
    ...
    //Convertir dicho array a json Again
    $jsonClientes = json_encode($aClientes);
    //Guardar el json en un archivo llamado archivo.txt Again
    file_put_contents("archivo.txt", $jsonClientes);
            ...
            <div class="col-6">
                ...
                <form action="" method="POST" enctype="multipart/form-data">
                    <div class="row">
                        ...
                        <div class="col-12  m y-2 form-group">
